...
class spectrainfo_fromascii:
	def __init__(self, filename, lam, flux):
		utdate, mjd, inst, ittime = self.infofromfilename(filename)
	...
	def infofromfilename(self, filename):
		sf = filename.split('.ascii')[0].split('_')
		utdate = sf[1]
		mjd = float(sf[2])
		inst = re.sub('-', "", sf[3])
                ittime = sf[4].split('.')[0]
		return utdate, mjd, inst, ittime

def dataFromAsci(ascifile, delimeter=" "):                              # read ascii file
...
def getphotometricdata_snecat(photofile):
	lcFile_o = open(photofile).readlines()[1:]
	data = []
	for row in lcFile_o:
		rs = row.split(',')
		err = float(rs[3]) if rs[3] != '' else 0.0 
		data.append(photometricexposureinfo(rs[5], float(rs[2]), err, float(rs[1]), ''))
	return data
...
def DopplerMag(l,fl,b,filename,dire=None):
	...
	#set a upper limit at 10% of the band  transmission
        print('Running DopplerMag on {}'.format(filename))
...
def main():
	...
	parser.add_option("--ff", "--fileformat", dest="fileformat", type=str, default="fits", help="spectra file format [fits, ascii]")
	parser.add_option("-a", "--runall", dest="runall", action="store_true", default=True, help="Run all files in the directory")
	...
	if fileformat not in ['fits', 'ascii']:
		sys.exit('file format must be fits or ascii')

	if run1 is not None:
		files = [x for x in os.listdir(os.getcwd()) if run1 in x]
	else: 
		files = [x for x in os.listdir(os.getcwd()) if fileformat in x and not 'PROCESS' in x]

	if filters is not "" and photometricdatafile is not "":
		try:
			photometricdata = getphotometricdata(photometricdatafile)
		except:
			photometricdata = getphotometricdata_snecat(photometricdatafile)
		
		data_filt = set([f.filter for f in photometricdata])

		filtreplace = []
		for f in filters:
			if f in data_filt:
				filtreplace.append(f)
			else:
				print("photometric datafile doesn't have data for filter " + f)
		filters = filtreplace
	...
	for file in files:
		...
		if fileformat == 'fits':
			...
			lam, flux = dataFromAsci(file, delimeter=None)
			info = spectrainfo_fromascii(file, lam, flux)
		if filters is not "" and photometricdatafile is not "":
			if fileformat == 'fits':
				asciifile = 'ascii_file.tmp' #re.sub('.fits', '.ascii', file)
			else:
				asciifile = 'ascii_file.tmp'
		...
		newfname = 'PROCESS.'+objectname + "_" + str(info.utdate) + "_" + str(info.mjd) + "_" + info.inst + "_" + str(info.ittime) + ".ascii.p"
		of = open(newfname, 'w')
